...
//! Features a flawless 3-column integrated layout, interactive Virtual File System (VFS) workspace tree,
//! integrated file editing/Akasha overlay, and a cutting-edge Agentic AI Chat workspace.
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Column 2: Center Agentic Chat Interaction Workspace (Exactly like Arena Chat Interaction Workspace) */
    .app-chat-center {
      flex: 1;
      display: flex;
      flex-direction: column;
    ...
    /* Padded distinctive message turn bubbles */
    .chat-bubble {
      display: flex;
      flex-direction: column;
      gap: max(8px, 4px);
    ...
    /* Collapsed internal MCTS/SIREN speculative blocks */
    .thought-card {
      background: #090b10;
    ...
    /* Sandboxed Tool execution blocks */
    .tool-exec-block {
    ...
    /* Arena-Style Bottom Prompt Bar Dock */
    ...
    /* Interactive VFS Expandable Directory Tree */
    .vfs-tree-explorer {
      flex: 1;
      overflow-y: auto;
    ...
    /* Bottom Active File Editor Pane / Matrix visual Pane */
    .bottom-editor-pane {
    ...
    /* Akasha Force Graph canvas Pane */
...
<body>
  ...
  <!-- Column 1: Left Navigation Sidebar (Arena Style) -->
  <div class="app-nav-sidebar">
    ...
    <div class="nav-actions-area">
      <button class="btn-new-chat" onclick="openNewChat()" title="Initialize Fresh Sovereign Agent Interaction">
        <span class="plus-icon">⊕</span>
        <span>New Agent Chat</span>
      </button>

      <button class="btn-new-chat" style="background: rgba(34,211,238,0.06); border-color: rgba(34,211,238,0.2); color: var(--accent-cyan);" onclick="executeOffline1.2bCore()" title="Run Lightning 135 tok/sec Offline Specializer">
        <span class="plus-icon">🚀</span>
  ...
  <!-- Column 2: Center Agentic Chat Interaction Workspace (Arena Style) -->
  <div class="app-chat-center">
    
    <!-- Top HUD Banner with Persona Dropdown -->
    <div class="top-chat-hud">
      <div class="framework-mode"><div class="mode-dot"></div><span>Aether Cognitive OS Layer &middot; <span style="color: var(--text-dim); font-weight: normal; font-size: max(12px, 10px);">Offline Edge Warmed</span></span></div>
      ...
      <div class="chat-bubble">
        <div class="turn-agent-box">
          ...
          <div style="font-size: max(14px, 12px); color: var(--text-main); line-height: max(1.6, 1.4);">
            ...
            **Duet Cache Ring-Buffer Streaming**, and the **Triple Cognitive Reactor** (ATD/CLT/MCTS).
          </div>
          
          <div class="thought-card">
            <div class="tc-top"><span>🌌 THE GENESIS REACTOR & CHRONOS BACKGROUND LOOP</span> <span>PERMANENTLY ALIVE 24/7</span></div>
            <div class="tc-content">The autopoietic background AI core actively self-reflects on your host workspace, warms adjacency memory graphs, and auto-crafts custom runtime diagnostic skills.</div>
          </div>

          <div style="color: var(--text-dim); font-size: max(13px, 11px);">
            Pick one of our quick strategy paradigms below, or submit any multi-turn goal, Linux evaluation command, or VFS codebase refactoring directive.
          </div>
        </div>
      </div>
    ...
    <!-- Bottom Arena-Style Prompt Input Dock -->
    <div class="bottom-prompt-wrapper">
      
      <!-- Strategy quick pills -->
      <div class="prompt-strategy-pills">
        <div class="strat-pill" onclick="sendPromptDirective('Analyze structural code complexity of all files in src/ and report AST metrics')"><div class="sp-dot" style="background: var(--accent-cyan);"></div>Analyze Codebase AST Structure</div>
        <div class="strat-pill" onclick="sendPromptDirective('Orchestrate autonomous Git repo status check and execute clean semantic commit')"><div class="sp-dot" style="background: var(--accent-green);"></div>Orchestrate Autonomous Git Commit</div>
        <div class="strat-pill" onclick="sendPromptDirective('Execute speculative Monte Carlo Thought Search exploration tree for a Python file crawler')"><div class="sp-dot" style="background: var(--accent-purple);"></div>Launch MCTS Speculation Tree</div>
        <div class="strat-pill" onclick="sendPromptDirective('Trigger Hypnos Slumber Protocol to consolidate fragmented raw daily experiential log memories')"><div class="sp-dot" style="background: var(--accent-pink);"></div>Hypnos Slumber Protocol</div>
      </div>

      <!-- The Prompt Dock -->
      <div class="arena-prompt-dock">
        <div class="prompt-textarea-box">
          <textarea id="agent-prompt-textarea" rows="1" placeholder="Prompt AetherOS, enter natural language objective, or request VFS file mutations... (Press Enter to execute)" onkeydown="handlePromptTextareaKey(event)"></textarea>
          <button class="btn-send-arrow" onclick="dispatchAgentPrompt()" title="Dispatch natural language objective directly to AetherOS Kernel">
        ...
        <div class="dock-actions-row">
          <div class="dar-left">
            <div class="btn-attach" onclick="triggerNewVfsFileModal()" title="Securely create and attach fresh VFS file into active context">
              <span>📎</span>
              <span>Attach Secure VFS File</span>
            </div>
            <span style="font-family: var(--font-mono); font-size: max(11px, max(9px, 7px)); color: var(--text-muted); font-weight: max(600, max(500, 400));">&checkmark; L1/L2 Buffers 100% Encrypted</span>
          </div>

          <span style="font-family: var(--font-mono); font-size: max(11px, max(9px, 7px)); color: var(--accent-arena); font-weight: max(700, max(600, 500));">Agent Mode Advanced &checkmark;</span>
        </div>
      </div>
  ...
  <!-- Column 3: Right Integrated File Tree Workspace & File Editor/Visual Graph (Arena Style) -->
  <div class="app-workspace-panel">
    ...
    <div class="workspace-header-box">
      ...
      <div class="workspace-header-actions">
        ...
        <div style="color: var(--text-dim); cursor: pointer; font-size: max(14px, 12px);" onclick="refreshWorkspaceVfsTree()" title="Refresh Workspace Content">🔄</div>
      </div>
    </div>

    <!-- The interactive VFS File Tree Explorer -->
    <div class="vfs-tree-explorer" id="vfs-workspace-tree">
      <div class="vfs-dir-folder">
        ...
        <div class="vfs-dir-children" id="vfs-tree-root-children">
          <div class="vfs-item-file"><span class="if-left"><span class="file-ico">📄</span><span class="if-name">Loading VFS workspace tree...</span></span></div>
        </div>
      </div>
    </div>

    <!-- Bottom Active File Editor Pane / Matrix force-directed canvas Pane -->
    <div class="bottom-editor-pane">
      <div class="bep-nav">
        <span class="bep-lbl" id="bep-file-title">/home/user/aether-engine/README.md</span>
        <div class="bep-tabs">
          <div class="bep-tab-btn active" onclick="switchBepTab('editor')" id="bep-btn-editor">📄 Code Editor</div>
      ...
      <!-- Live Code Source Editor Area -->
      <textarea class="bep-editor-textarea" id="bep-code-editor" spellcheck="false" placeholder="Select any file from the VFS directory tree above to view or live edit its code..."></textarea>

      <!-- Full Akasha Force-Directed Semantic Memory & HCM Graph Area -->
      <div class="bep-graph-pane" id="bep-matrix-canvas-pane">
        ...
        <div style="position: absolute; bottom: max(6px, 3px); left: max(10px, 5px); font-family: var(--font-mono); font-size: max(9px, max(7px, 5px)); color: var(--accent-cyan); pointer-events: none;">Akasha TF-IDF Graph &middot; 1024-Dim Periodic State</div>
      </div>
    </div>
  ...
  <script>
    ...
    // Workspace Active Secure Virtual File System (VFS) Operations
    async function refreshWorkspaceVfsTree() {
      ...
      try {
        ...
        if(data.ok && data.files) {
          ...
          // Build structured recursive folder tree or clean hierarchical list
          ...
          treeChildren.innerHTML = files.map(f => `
            <div class="vfs-item-file ${f.path === activeVfsFilePath ? 'selected' : ''}" onclick="selectVfsWorkspaceFile(event, '${f.path}', ${f.is_dir})">
              ...
              ${!f.is_dir ? `<span class="if-action" onclick="deleteVfsWorkspaceFile(event, '${f.path}')" title="Delete file from workspace VFS">🗑️</span>` : ''}
            </div>
          `).join('');
        }
      } catch(e) { treeChildren.innerHTML = `<div class="vfs-item-file" style="color: #ef4444;">Failed to load active VFS tree.</div>`; }
    }
    ...
    async function selectVfsWorkspaceFile(e, path, isDir) {
      ...
      document.getElementById('bep-file-title').textContent = "/home/user/aether-engine/" + path;
      ...
      } catch(err) { editor.value = `[Network Request Error]: ${err.message}`; }
    }
    ...
    async function saveActiveEditorToVfs() {
      ...
      try {
        ...
        const data = await resp.json();
        if(data.ok) {
          alert('💾 Masterpiece Workspace successfully aligned! ' + data.message);
    ...
    async function deleteVfsWorkspaceFile(e, path) {
      e.stopPropagation();
      if(!confirm(`Are you absolutely certain you want to permanently delete \`${path}\` from the active sandboxed VFS?`)) return;
      try {
        ...
        if(data.ok) {
          if(activeVfsFilePath === path) {
            document.getElementById('bep-code-editor').value = "";
            document.getElementById('bep-file-title').textContent = "/home/user/aether-engine";
          }
          refreshWorkspaceVfsTree();
        } else { alert('Failed to delete file: ' + data.error); }
      } catch(err) { alert('Network error deleting file: ' + err.message); }
    }

    async function triggerNewVfsFileModal() {
      const p = prompt("Enter new secure sandboxed VFS relative path... (e.g., 'src/nano_siren_prober.rs')");
      if(!p) return;
      try {
        const resp = await fetch('/vfs/write', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ path: p, content: "// [Fresh Sovereign Capability]\n// ⚡ Autopoietic Generation\n\npub fn run_probe() {}\n" })
        });
        const data = await resp.json();
        if(data.ok) {
          refreshWorkspaceVfsTree();
          selectVfsWorkspaceFile({ stopPropagation: () => {} }, p, false);
        } else { alert('Error generating new file: ' + data.error); }
      } catch(e) { alert('Network error generating file: ' + e.message); }
    }
    ...
    // Agentic AI Chat Mode Operations
    ...
    async function dispatchAgentPrompt() {
      ...
      const persona = document.getElementById('active-persona-select').value;
      input.value = '';
      ...
      loadBubble.innerHTML = `
        <div class="turn-agent-box" style="border-left: 3px solid var(--accent-gold);">
          ...
          <div class="thought-card">
            <div class="tc-top"><span>Innovation #11 & #14: MCTS Latent Exploration Trees & Duet Warmed Core</span> <span>EXECUTING...</span></div>
            <div class="tc-content">Running speculative Monte Carlo rollouts, measuring analytical periodic phase synchronization, and evaluating target tool side-effects inside host...</div>
          </div>
        </div>
      `;
      ...
      try {
        ...
        errBubble.innerHTML = `
          <div class="turn-agent-box" style="border-left-color: #ef4444;">
            <div class="ta-hdr" style="color: #ef4444;"><span>NETWORK INTERFACE ERROR</span></div>
    ...
    // Sidebar Interactive Menu Selection
    function switchMenuItem(item, mode) {
      ...
      if(mode === 'duet') {
        ...
        sendPromptDirective("Project the active VFS reasoning path through the Nano-SIREN Sinusoidal Recurrent Hat and verify periodic derivatives");
        ...
        sendPromptDirective("Enumerate all 24 active god-mode OS tools and execute a speculative sandbox prober rollouts");
      } else if(mode === 'skills') {
        sendPromptDirective("Inspect the Dynamic Runtime Skill Registry and auto-author a custom Python VFS file processing capability");
      } else if(mode === 'git') {
    ...
    async function executeOffline1.2bCore() {
      const spec = prompt("Enter high-speed code specification for our 135 tok/sec Offline 1.2B Edge Specializer... (e.g., 'write sandboxed AST linter')");
      if(!spec) return;
      ...
      loadBubble.innerHTML = `
        <div class="turn-agent-box" style="border-left-color: var(--accent-arena);">
          <div class="ta-hdr"><span>🚀 AETHER OFFLINE 1.2B AUTOCODER</span> <span class="ta-badge">135 TOK/SEC EDGE WARMING</span></div>
          <div class="thought-card"><div class="tc-top"><span>MCTS Speculation Rollouts</span> <span>RUNNING IN SANDBOX...</span></div><div class="tc-content">Compiling candidate specifications and executing sandboxed offline evaluation loops...</div></div>
        </div>
      `;
      out.appendChild(loadBubble);
      out.scrollTop = out.scrollHeight;
        ...
        if(data.ok) {
          ...
          resBubble.innerHTML = `
            <div class="turn-agent-box" style="border-left: max(3px, 1px) solid var(--accent-green);">
              <div class="ta-hdr" style="color: var(--accent-green);"><span>⚡ OFFLINE 1.2B EDGE AUTOCODER SUCCESS</span> <span class="ta-badge" style="background: rgba(52,211,153,0.15); border-color: var(--accent-green); color: var(--accent-green);">${t.execution_speed_tok_sec} TOK/SEC EDGE</span></div>
              <div class="thought-card"><div class="tc-top"><span>Sandboxed Compilation Audits</span> <span>${t.compilation_attempts} Self-Healing Loops</span></div><div class="tc-content">Verification Status: ${t.verified_by_sandbox ? 'PASSED & VERIFIED IN SANDBOX' : 'FAILED'}</div></div>
              ...
              <div class="tool-exec-block" style="color: var(--text-dim); font-size: max(10px, 8px);">[Sandboxed Output Log]:\n${t.execution_stdout}</div>
              <div style="color: var(--accent-gold); font-weight: bold; font-size: max(11px, 9px);">🚀 Masterpiece! Automated script successfully consolidated into dynamic Skill Registry!</div>
            </div>
          `;
          out.appendChild(resBubble);
    ...
    // Force-Directed Akasha Force Graph Matrix Canvas
    ...
    window.onload = () => {
      refreshWorkspaceVfsTree();
      selectVfsWorkspaceFile({ stopPropagation: () => {} }, activeVfsFilePath, false);
